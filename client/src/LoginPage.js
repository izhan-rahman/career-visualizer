  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success && data.role === loginType) {
        ...
        onLoginSuccess(data.role); // Pass the role up to App.js
        ...
        >
          ...
          <div className="flex w-full bg-black bg-opacity-20 rounded-full p-1">
            <button
              ...
              className={`w-1/2 py-2 rounded-full text-white ${loginType === 'user' ? 'bg-purple-600' : 'opacity-70'}`}
            >
              User
            </button>
            <button
              onClick={() => setLoginType('admin')}
              className={`w-1/2 py-2 rounded-full text-white ${loginType === 'admin' ? 'bg-purple-600' : 'opacity-70'}`}
            >
              Admin
            </button>
          </div>
